...
namespace DiscordClient.Handlers
{
    public class CommandHandler
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            foreach (ModuleInfo mf in _commands.Modules)
            {
                if (mf.Name != "Help")
                {
                    ...
                    {
                        Debug.WriteLine($"Added Command:\t{ci.Name}");
                        commandsModels.Add(ci);
                    }
                }
            }
            Debug.WriteLine($"Total Commands:\t{_commands.Modules.Count()}");
        ...
        private Task HandleCommandAsync(SocketMessage socketMessage)
        {
            ...
            if (!message.HasStringPrefix(Bot.AppData.Prefix, ref argPos))
                return Task.CompletedTask;
            ...
            if (Bot.AppData.BlacklistedChannels != null)
            {
                var blacklistedChannelCheck = from a in Bot.AppData.BlacklistedChannels
